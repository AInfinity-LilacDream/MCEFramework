...
public class CrazyMinerFuncImpl {
    ...
    public static void loadConfig(CrazyMiner game) {
        ...
        plugin.getLogger().info("游戏区域中心: (" + crazyMinerConfigParser.getGameAreaCenterX() + ", "
                + crazyMinerConfigParser.getGameAreaCenterZ() + ")");
        plugin.getLogger().info("游戏区域大小: " + crazyMinerConfigParser.getGameAreaSizeX() + "x"
                + crazyMinerConfigParser.getGameAreaSizeZ());
        ...
        plugin.getLogger().info("中心空缺区域大小: " + crazyMinerConfigParser.getCenterClearAreaSize() + "x"
                + crazyMinerConfigParser.getCenterClearAreaSize());
        ...
        plugin.getLogger()
                .info("外圈方块类型数: " + (game.getOuterRingBlocks() != null ? game.getOuterRingBlocks().size() : 0));
        plugin.getLogger()
                .info("内圈方块类型数: " + (game.getInnerRingBlocks() != null ? game.getInnerRingBlocks().size() : 0));
     ...
     * 强制重新生成最外层基岩墙，确保完整性
     */
    private static void reinforceOuterBedrockWall(World world, int centerX, int centerZ, int y, int height,
            int halfSizeX, int halfSizeZ) {
     ...
     */
    private static void createSpawnHoles(World world, int centerX, int centerZ, int y, int height, int halfSizeX,
            int halfSizeZ, CrazyMiner game) {
     ...
     * 创建单个7x5x5出生点洞穴
     */
    private static void createSpawnHole(World world, int centerX, int centerY, int centerZ,
    ...
    public static void generateRandomBlocks(CrazyMiner game) {
        ...
        plugin.getLogger().info("开始替换基岩方块，范围: x(" + (centerX - halfSizeX) + "~" + (centerX + halfSizeX) + "), y(" + y
                + "~" + (y + height - 1) + "), z(" + (centerZ - halfSizeZ) + "~" + (centerZ + halfSizeZ) + ")");
